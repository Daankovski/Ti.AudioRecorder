...
#define TAG "com.daan.audiorecorder"

using namespace v8;

static Persistent<Object> bindingCache;

static void Audiorecorder_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Audiorecorder.getBinding requires 1 argument: binding");
...
static void Audiorecorder_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Audiorecorder_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Audiorecorder_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
...
Java_com_daan_audiorecorder_AudiorecorderBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.daan.audiorecorder", &AudiorecorderBinding);
